...
gait_data=load('C:\Users\zzs\Desktop\chenhao\10分钟帧数据仅步态数据\zhuyunchao_02.csv');
z6=gait_data(:,18);
...
INDEX=[];
%% 平底鞋
point_toe=[-5.8584	10.6317	-364.7494];
point_h=[-57.7245	5.6188	-449.8804];
...
%                 plot(Z(j,:))
        ...
        try
            [pks_loc_p,locs_loc_p]=findpeaks(Z(j,locs_loc(1):end),"MinPeakDistance",10);

        catch
            continue
        end
        [pks_loc_p,locs_loc_p]=Eliminate_adjacent_peaks(pks_loc_p,locs_loc_p);
        locs_loc_p=locs_loc_p+locs_loc(1);

        %% 如果光滑处理过后，最后一个局部最大值找不到了，
        if size(locs_loc,2)==2&&size(locs_loc_p,2)==1
        ...
        pks_loc_p(find( locs_loc_p<40))=[];
        locs_loc_p( locs_loc_p<40)=[];
        if j==1
            ...
            if size(locs_loc_p,2)==2 && size(locs_loc,2)==2 && locs_loc(2)>locs_loc_p(1)...
                    && locs_loc(2)<locs_loc_p(2) && locs_loc_p(2)>locs_loc(1) && ...
                    locs_loc_p(1)>locs_loc(1) && locs_loc_p(2)-locs_loc(2)>0&& ...
            ...
            elseif  size(locs_loc,2)==1 &&size(locs_loc_p,2)==1&&locs_loc<locs_loc_p
                in=find_inflection_point1(Z(j,locs_loc(1):locs_loc_p(end)));
                in=Eliminate_adjacent_peaks_de2(in);
                in(diff(in)<4)=[];
                in=in+locs_loc(1);
                ini_1=find_inflection_point3(Z(j,locs_loc(1):locs_loc_p(end)));
                ini_1=Eliminate_adjacent_peaks_de2(ini_1);
                ini_1(diff(ini_1)<4)=[];
                ini_1=ini_1+locs_loc(1);
