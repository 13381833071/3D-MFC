...
%         plot(normlity_percent2(-(LTOE(Ltoe_off:locs_l(end),2)-LAnkleJC_all(Ltoe_off:locs_l(end),2))./57),'k','LineWidth',2,'Marker','*','MarkerEdgeColor','k','MarkerFaceColor','w','LineStyle','-')% 踝关节角度的平均敏感度
%         hold on;
%         plot(normlity_percent2((LTOE(Ltoe_off:locs_l(end),2)-LKneeJC_all(Ltoe_off:locs_l(end),2))./57),'k','LineWidth',2,'Marker','diamond','MarkerEdgeColor','k','MarkerFaceColor','w','LineStyle','-')% 膝关节角度的平均敏感度
%         hold on;
%         plot(normlity_percent2(-(LTOE(Ltoe_off:locs_l(end),2)-LIAS(Ltoe_off:locs_l(end),2))./57),'k','LineWidth',2,'MarkerEdgeColor','k','MarkerFaceColor','w','LineStyle','-')% 踝关节角度的平均敏感度
%         line([54,54],[-8,8],'color','k','linestyle','-')
%         % title('踝、膝、髋关节角度对脚趾间隙的','Fontsize',16,'FontWeight','bold','Fontname','微软雅黑')%Foot-shoe roll-over shapes Ankle-foot roll-over shapes
%         figure_FontSize=14;
%         set(gca, 'Fontname','微软雅黑','Fontsize', 14);
%         xlabel('% Swing phase','fontsi',14,'FontWeight','bold','Fontname','微软雅黑');
%         ylabel ('脚趾间隙敏感度(mm/degree)','Fontname','微软雅黑','fontsi',14,'FontWeight','bold');%,'posit
%         set(get(gca,'YLabel'),'FontSize',figure_FontSize,'Vertical','middle');
%         set(findobj('FontSize',14),'FontSize',figure_FontSize,'Fontname','微软雅黑');
%         set(gca, 'Box', 'off');
%         % hold on;
%         % plot(normlity_percent2(LTOE(Ltoe_off:locs_l(end),3)./100))
%         % scatter(0,0,80,'k','filled','o','MarkerEdgeColor','k','MarkerEdgeAlpha',1,'AlphaData',0.8,'MarkerFaceColor','k','MarkerFaceAlpha',0.4)
%         legend('踝背伸','膝屈曲','髋屈曲','Orientation','horizontal','FontName','微软雅黑','Fontsize',14,'Location','NorthEast');
%         ylim([-8 10]);
%         % plot(-(LTOE(Ltoe_off:locs_l(end),3)-LFAL(Ltoe_off:locs_l(end),3)))
        ...
        [ LROLL_OVER_X_F,LROLL_OVER_Y_F] = create_ROLL_OVER_F(LFCC,LTOE,LAnkleJC_all,Lcop_x,Lheel_strike,Rheel_strike);

        [ LROLL_OVER_X_AF,LROLL_OVER_Y_AF] = create_ROLL_OVER_AF(LFCC,LTOE,LAnkleJC_all,LKneeJC_all,Lcop_x,Lheel_strike,Rheel_strike);

        [ LROLL_OVER_X_KAF,LROLL_OVER_Y_KAF] = create_ROLL_OVER_KAF(LAnkleJC_all,Lcop_x,Lheel_strike,Rheel_strike, ...
            LTH1,LTH2,LTH3,LTH4,LSHK1,LSHK2,LSHK3,LSHK4,LIAS,RIAS,LIPS,RIPS,LFLE,LFME,LFAL,LTAM);

        data2 = [LROLL_OVER_X_F LROLL_OVER_Y_F LROLL_OVER_X_AF LROLL_OVER_Y_AF LROLL_OVER_X_KAF LROLL_OVER_Y_KAF];
        xlswrite(fullfile( maindir, subdir( i ).name,'Roll_Over_Shape2',strcat(strrep(fiels( j ).name,'.csv',''),'L','.xls')),data2);
